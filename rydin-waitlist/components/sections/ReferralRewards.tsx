      ...
      <div className="mx-auto max-w-6xl">
        ...
        <Reveal>
          <h2 className="mt-6 max-w-2xl text-display-sm font-bold text-balance text-slate-900">
            Carpooling only works if your route fills up.
          </h2>
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          {/* ---------- Tier ladder ---------- */}
          <Reveal className="rounded-3xl bg-paper-raise p-6 shadow-card ring-hairline sm:p-8">
            <div className="flex items-baseline justify-between">
              <p className="text-xs font-medium text-slate-500">
                {pass ? "Your progress" : "Reward ladder"}
              </p>
              <p className="font-mono text-xs text-slate-500" data-numeric>
                {referrals}/{maxReferrals} invites
              </p>
            </div>

            {/* Rail */}
            <div className="relative mt-4 h-2 rounded-full bg-slate-900/[0.08]">
              <motion.div
                    ...
                    <span
                      className={cn(
                        ...
                        reached
                          ...
                          : "bg-paper-sunken text-slate-400 ring-slate-900/[0.08]",
                      )}
                    >
                      <Icon className="h-5 w-5" strokeWidth={2} />
                    </span>
                    <div className="min-w-0 flex-1 border-b border-slate-900/[0.06] pb-4 last:border-0 last:pb-0">
                      <div className="flex flex-wrap items-baseline gap-x-2">
                        <span className="font-mono text-xs font-semibold text-brand-600">
                          {tier.referrals} invite{tier.referrals === 1 ? "" : "s"}
                        </span>
                        <span
                          className={cn(
                            "text-sm font-semibold",
                            reached ? "text-slate-900" : "text-slate-700",
                          )}
                        >
                          {tier.title}
                        </span>
                        {reached && (
                          <span className="rounded-full bg-brand-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-brand-700 ring-1 ring-inset ring-brand-500/20">
                            Unlocked
                          </span>
                        )}
                      </div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-slate-900/[0.07] pt-5">
              ...
              {!pass && (
                <button
                  type="button"
                  onClick={focusForm}
                  className="ml-auto text-xs font-semibold text-brand-700 underline-offset-4 hover:underline focus-visible:ring-2 focus-visible:ring-brand-500/60"
                >
                  Get your link →
                </button>
              )}
          ...
          <Reveal
            delay={0.08}
            className="rounded-3xl bg-paper-raise p-6 shadow-card ring-hairline sm:p-7"
          >
                ...
                <StaggerItem key={entry.rank}>
                  <div
                    className={cn(
                      ...
                      entry.rank === 1
                        ? "bg-gold-50 ring-1 ring-inset ring-gold-400/30"
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        entry.rank === 1
                          ...
                          : "bg-paper-sunken text-slate-500 ring-1 ring-inset ring-slate-900/[0.06]",
                      )}
                    >
                      {entry.rank}
                    </span>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate font-mono text-[10px] uppercase tracking-wide text-slate-400">
                        {entry.affiliation}
                      </p>
                    </div>
            ...
            <p className="mt-5 border-t border-slate-900/[0.07] pt-4 text-[11px] leading-relaxed text-slate-400">
