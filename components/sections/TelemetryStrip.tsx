  ...
  return (
    <section className="relative px-4 py-10 sm:px-6 lg:py-14">
      <Reveal className="mx-auto max-w-6xl">
        ...
        */}
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-3xl bg-slate-900/[0.08] dark:bg-slate-50/[0.08] shadow-card ring-hairline lg:grid-cols-4">
          <Cell
...
function Cell({
  ...
  return (
    <div className="bg-paper-raise dark:bg-slate-900 p-5 sm:p-6">
      <div className="flex items-center gap-2">
        <span className={tone === "amber" ? "text-gold-500" : "text-brand-600 dark:text-brand-400"}>
          {icon}
        </span>
      ...
      <p
        className={`mt-3 font-mono text-2xl font-bold tracking-tight sm:text-3xl ${
          tone === "amber" ? "text-gold-600" : "text-slate-900 dark:text-slate-50"
        }`}
        data-numeric
      >
        {value}
      ...
      {footnote && (
        <p className="mt-1 text-[11px] leading-snug text-slate-400 dark:text-slate-500">{footnote}</p>
      )}
    </div>
  );
}
