    ...
    <section id="savings" className="relative px-4 py-16 sm:px-6 lg:py-24">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          {/* ---------- Controls ---------- */}
          <Reveal className="rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-card sm:p-7">
            {/* Distance */}
            <div>
              <div className="flex items-baseline justify-between">
                ...
                <span
                  className="font-mono text-sm font-semibold text-brand-700 dark:text-brand-300"
                  data-numeric
                >
                  {distance} km
                </span>
          ...
          <div className="flex flex-col gap-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Reveal
                delay={0.05}
                className="relative rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-card"
              >
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
                  Commercial cabs / taxis
                </p>
                <p
                  className="mt-3 font-mono text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50"
                  data-numeric
                >
                  <AnimatedNumber
                ...
                <p className="mt-1 text-[11px] text-slate-400 dark:text-slate-500">per month</p>
                <p className="mt-4 text-[11px] leading-snug text-slate-500 dark:text-slate-400">
              ...
              <Reveal
                delay={0.1}
                className="relative overflow-hidden rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-card"
              >
                <div
                  aria-hidden="true"
                  className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-brand-200/50 blur-3xl"
                />
                <p className="relative text-xs font-medium text-brand-700 dark:text-brand-300">
                  With Rydin cost-sharing
                </p>
                <p
                  className="relative mt-3 font-mono text-3xl font-bold tracking-tight text-brand-800 dark:text-brand-200"
                  data-numeric
                >
                  <AnimatedNumber
                ...
                <p className="relative mt-1 text-[11px] text-brand-600 dark:text-brand-400">
                  per month
                </p>
                <p className="relative mt-4 text-[11px] leading-snug text-brand-700 dark:text-brand-300/80">
            ...
            <Reveal
              delay={0.15}
              className="rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-card sm:p-7"
            >
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="flex items-center gap-1.5 text-xs font-medium text-brand-700 dark:text-brand-300">
                    <TrendingDown className="h-3.5 w-3.5" strokeWidth={2.4} />
                    You pocket
                  </p>
                  <p
                    className="mt-1.5 font-mono text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-50 sm:text-5xl"
                    data-numeric
                  >
                    <AnimatedNumber
                ...
                <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-3 text-slate-900 dark:text-slate-100 shadow-card">
                  {result.costsMoreThanSolo ? (
                    ...
                    <>
                      <p
                        className="font-mono text-2xl font-bold text-brand-700 dark:text-brand-300"
                        data-numeric
                      >
                        <AnimatedNumber
...
function Segmented<T extends string | number>({
  ...
  return (
    <div
      className="grid gap-1.5 rounded-2xl bg-paper-sunken dark:bg-slate-800 p-1.5 ring-hairline-strong"
      style={{
            ...
            <span className="relative text-center leading-tight">
              ...
              {opt.sub && (
                <span
                  className={cn(
                    "block text-[10px]",
                    active ? "text-brand-50/90" : "text-slate-400 dark:text-slate-500",
                  )}
                >
                  {opt.sub}
                </span>
